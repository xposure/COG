...
namespace OpenTK
{
    public static class Utility
    {
        ...
        /// <param name="d2"></param>
        /// <returns></returns>
        public static Tuple2<bool, Real, Real> Intersect(Ray3 ray, AxisAlignedBox3 box)
        {
            if (box.IsNull)
            {
                return new Tuple2<bool, Real, Real>(false, Real.NaN, Real.NaN);
            }

            if (box.IsInfinite)
            {
                return new Tuple2<bool, Real, Real>(true, Real.NaN, Real.PositiveInfinity);
            }
            ...
            return new Tuple2<bool, Real, Real>(true, start, end);
        }
        ...
        //public static float Lerp(Real x, Real y, Real amount)
        //{
        //    return x + (y - x) * amount;
        //}

        public static float Lerp(Vector2 x, Vector2 y, float amount)
        {
            var v0 = Lerp(x.X, y.X, amount);
            var v1 = Lerp(x.Y, y.Y, amount);
            ...
            where T : IRadixKey
        {
            ...
            for (int c = 0, shift = 0; c < groups; c++, shift += r)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    var shiftKey = a[i].Key >> shift;
                    var shiftKeyMask = shiftKey & mask;
                    count[shiftKeyMask]++;
                }
